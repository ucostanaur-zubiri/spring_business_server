...
class SpringBusinessServerApplicationTests {
  ...
  public void testLoginWithUsernameAndOtp() throws Exception {
    when(authenticationServerProxy.sendOTP("bill", "5555")).thenReturn(true);
  ...
  public void testRequestWithAuthorizationHeader() throws Exception {
    ...
    var authorizationHeaderValue = mvc
        .perform(get("/login").servletPath("/login").header("username", "bill").header("code", "5555")).andReturn()
        .getResponse().getHeader("Authorization");

    mvc.perform(get("/test").header("Authorization", authorizationHeaderValue)).andExpect(status().isOk());
  }
